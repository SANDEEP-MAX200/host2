...
export default function Dashboard() {
  ...
  const fetchAnalytics = async (start, end) => {
    try {
      const res = await fetch(`${API_URL}/user/analytics/period?startDate=${start}&endDate=${end}`, { credentials: "include" });
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const endpoint = email ? `${API_URL}/admin/users/view/${encodeURIComponent(email)}` : `${API_URL}/dashboard`;
